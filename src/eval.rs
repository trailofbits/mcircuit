            ...
            match step {
                CombineOperation::GF2(gate) => {
                    for wire in gate.inputs().chain(gate.outputs()) {
                        ...
                        let mut current_scope: &str = "bool_scope";
                        ...
                        while let Some(t) = scope_tokens.next() {
                            if scope_tokens.peek().is_some() {
                                ...
                                bool_scopes
                                    .entry(current_scope.into())
                                    .or_insert_with(HashSet::new)
                                    .insert(ScopeEntry::Terminal((backref.clone(), wire)));
                            }
                        }
                    }
                }
                CombineOperation::Z64(gate) => {
                    for wire in gate.inputs().chain(gate.outputs()) {
                        ...
                        let mut current_scope: &str = "arith_scope";
                    ...
                    while let Some(t) = scope_tokens.next() {
                        if scope_tokens.peek().is_some() {
                            ...
                            arith_scopes
                                .entry(current_scope.into())
                                .or_insert_with(HashSet::new)
                                .insert(ScopeEntry::Terminal((backref.clone(), *dst)));
                        }
                    }
